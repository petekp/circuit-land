...
no layer set them. An explicit `model` or `effort` in any selection override
wins over the dial. Set the dial itself with `--power` on a run or
`defaults.power` in config; per-connector tier tables live under
`power_tiers` (see [Connectors](/docs/configuration/connectors)).

...
Connectors differ in one capability that matters for [tool scope](/docs/concepts/tool-scope): whether they can hard-enforce a step's allowed tools. Only `claude-code` can, through its session tool list, so an enforced scope is a real wall there. On `codex`, `cursor-agent`, and custom connectors an enforced scope downgrades to guidance, and the [trace](/docs/concepts/evidence-and-runs) records the downgrade. Enforcement applies to the implementer step, the one that writes code.
...
| `capabilities.tool_scope` | `none` (the only value accepted for a custom connector) |
...
- Custom connectors must declare `tool_scope: none`. Hard-enforcing a tool allow-list needs a builtin connector that can restrict its worker's tools.

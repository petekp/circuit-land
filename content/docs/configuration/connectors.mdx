...
concrete model or reasoning effort. The builtins ship with defaults:
`claude-code` maps the tiers to Anthropic's small, middle, and big model
aliases; `codex` maps them to reasoning effort. A custom connector has no
table until you declare one under the top-level `power_tiers` key:

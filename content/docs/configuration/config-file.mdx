...
| `defaults` | Baseline selection (`defaults.selection`) and the power dial default (`defaults.power`: `auto`, `low`, `medium`, or `high`) for every run. See [Selection](/docs/configuration/selection). |

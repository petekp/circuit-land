...
Create a new custom flow from the Build template, validate it, and optionally publish it. Each create writes a draft alongside a custom-flow descriptor; `--publish --yes` promotes the draft into a runnable flow, skill, and slash command.
...
| `--template-flow-root <path>` | Root containing the `build/circuit.json` template to start from. | |

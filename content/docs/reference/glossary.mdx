...
- **Power** — the model dial: how much model each worker run gets. Set with `--power`: `low`, `medium`, or `high`. Never names models; per-connector tier tables translate it at selection time.
...
- **Receipt** — the end-of-run line reporting depth, power, worker runs, escalations, and check results.

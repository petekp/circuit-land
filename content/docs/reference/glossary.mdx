...
- **Block**: a reusable kind of work that can appear in a schematic, declaring
  a typed input contract, a typed output contract, and checks. The same block
  appears across flows, such as a grounding read or an independent review. See
...
- **Micro-harness**: the small, purpose-built harness Circuit assembles around
  a single step: a role, a model and effort resolved from that role and the
  power dial, a tool scope, injected skills, and only the inputs its contract
  declares. Built fresh for that step alone, then discarded. See
  [The micro-harness](/docs/concepts/micro-harness).
